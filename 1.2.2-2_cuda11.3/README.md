...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-2_cuda11.3
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    -v /local/dir:/container/dir \
    -it waikatodatamining/mmsegmentation:1.2.2-2_cuda11.3
  ```
...
```bash
docker build -t mmsegmentation:1.2.2-2_cuda11.3 .
```
...
* Tag
  ...
  ```bash
  docker tag \
    mmsegmentation:1.2.2-2_cuda11.3 \
    public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-2_cuda11.3
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-2_cuda11.3
  ```
...
* Tag
  ...
  docker tag \
    mmsegmentation:1.2.2-2_cuda11.3 \
    waikatodatamining/mmsegmentation:1.2.2-2_cuda11.3
  ```
  
* Push

  ```bash
  docker push waikatodatamining/mmsegmentation:1.2.2-2_cuda11.3
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
