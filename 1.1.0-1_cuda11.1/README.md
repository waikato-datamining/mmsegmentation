...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.1.0-1_cuda11.1
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    -v /local/dir:/container/dir \
    -it waikatodatamining/mmsegmentation:1.1.0-1_cuda11.1
  ```

### Build local image

* Build the image from Docker file (from within /path_to/mmsegmentation/1.1.0-1_cuda11.1)
...
```bash
docker build -t mmsegmentation:1.1.0-1_cuda11.1 .
```
...
* Tag
  ...
  ```bash
  docker tag \
    mmsegmentation:1.1.0-1_cuda11.1 \
    public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.1.0-1_cuda11.1
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.1.0-1_cuda11.1
  ```
...
* Tag
  ...
  docker tag \
    mmsegmentation:1.1.0-1_cuda11.1 \
    waikatodatamining/mmsegmentation:1.1.0-1_cuda11.1
  ```
  
* Push

  ```bash
  docker push waikatodatamining/mmsegmentation:1.1.0-1_cuda11.1
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
