...
```
v1.2.2
c685fe6767c4cadf6b051983ca6208f1b9d1ccb8
```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-1_cpu
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G \
    -v /local/dir:/container/dir \
    -it waikatodatamining/mmsegmentation:1.2.2-1_cpu
  ```

### Build local image

* Build the image from Docker file (from within /path_to/mmsegmentation/1.2.2_cpu)
...
```bash
docker build -t mmsegmentation:1.2.2-1_cpu .
```
...
* Tag
  ...
  ```bash
  docker tag \
    mmsegmentation:1.2.2-1_cpu \
    public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-1_cpu
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-1_cpu
  ```
...
* Tag
  ...
  docker tag \
    mmsegmentation:1.2.2-1_cpu \
    waikatodatamining/mmsegmentation:1.2.2-1_cpu
  ```
  
* Push

  ```bash
  docker push waikatodatamining/mmsegmentation:1.2.2-1_cpu
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
