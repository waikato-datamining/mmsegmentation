...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-2_cpu
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G \
    -v /local/dir:/container/dir \
    -it waikatodatamining/mmsegmentation:1.2.2-2_cpu
  ```
...
```bash
docker build -t mmsegmentation:1.2.2-2_cpu .
```
...
* Tag
  ...
  ```bash
  docker tag \
    mmsegmentation:1.2.2-2_cpu \
    public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-2_cpu
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmsegmentation:1.2.2-2_cpu
  ```
...
* Tag
  ...
  docker tag \
    mmsegmentation:1.2.2-2_cpu \
    waikatodatamining/mmsegmentation:1.2.2-2_cpu
  ```
  
* Push

  ```bash
  docker push waikatodatamining/mmsegmentation:1.2.2-2_cpu
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
