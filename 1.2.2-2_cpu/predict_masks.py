...
def mask_to_polygon(mask, mask_threshold=0.1, mask_nth=1, view=None, view_margin=5, fully_connected='low'):
    ...
    :type mask_nth: int
    :param view: the view of the mask to use in order to speed up polygon calculation, e.g., a bounding box (x0, y0, x1, y1), with x1/y1 being included
    :type view: tuple
    ...
    :return: the polygon(s) describing the detected object(s): each contour is an ndarray of shape (n, 2), consisting of n (x, y) coordinates along the contour
    :rtype: np.ndarray
    """
